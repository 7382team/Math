...
## ⚡ 給遊戲端 TL;DR（2026-07-13，先看這段）

**現況**：內容大幅擴充到 **110 個可玩單元 / 7 科（國一上）**，全部照三家出版社（康軒/南一/翰林）章節整理。

**你（遊戲端）幾乎不用改：**
- 所有單元（含新增的國文課文、英文課次、歷史各課…）**都用同一套 iframe＋postMessage 契約**（§2/§3）。→ **部署後新單元自動能讀、能玩，零額外工作。** 你截圖裡國文單元已正常顯示，就是證明。
- `units.json` 只是**新增欄位**、向下相容：`courses[].chapters`（大章→小節，家長端用；遊戲端可忽略）、少數 unit 多了 `author`。照舊用 `id`/`file`/`ready`/`subject` 即可，**記得用 `ready` 過濾**（`ready:false` 別顯示）。
- ⚠️ 遊戲只顯示**家長已開**的單元（`settings.selected_units`）。新單元要家長在 parent.html 開了才會出現在副本。

**你唯一需要「寫 code」接的一件事＝段考複習組**（你截圖底部「這組的單元遊戲還沒接」）：
- 家長設定存在 `settings.exam_groups`，格式：`[{ id, name, unit_ids:[...], subject, active }]`。
- 要做的：取一個 `active` 的組 → 把 `unit_ids`（先用 `ready` 過濾）各單元的題目**逐題交錯**成一場綜合測驗。
- 每個單元 HTML 支援 `?exam=1` 鉤子（會隱藏單元自己的「下一題／難度分頁」，交給你外層 runner 控制；判分照常用 postMessage 回報）。**題庫側已有一個網頁版 runner 可參考**（index.html 內），你照著在遊戲裡做一個即可。詳見 `EXAM_GROUP_SPEC.md`。

**一句話**：新內容你不用管、部署完就有；只有「段考綜合測驗」要你這邊實作一個 runner。
...
### 14f.【2026-07-13 追加】英文**七下（第二冊）**三家課次完成
- 新增 **21 個英文七下 unit**（`eng-k2-*`/`eng-h2-*`/`eng-n2-*`，各家 Starter＋U1–6），依各課文法重點出題（頻率副詞/How often、can・may・must、be動詞過去式、一般動詞過去式、Why・because・so、交通工具與問路），已生成＋審題、全部 `ready:true`。
- 三家英文 course 的 `chapters` 現在包含四個大章：七上「第一冊 課次」「文法總複習」＋七下「第二冊 課次」「文法總複習（下）」（後者沿用既有 `eng-past`/`eng-future-modal`/`eng-wh-adverb`）。
- 全庫：**131 個可玩單元、0 準備中**。英文上下學期皆完備。

### 14g.【2026-07-13 追加】國文**七下經典選文** 15 課完成
- 新增 15 個國文七下 unit（`chi-mulan`木蘭詩、`chi-zuoyouming`座右銘、`chi-loushiming`陋室銘、`chi-ailianshuo`愛蓮說、`chi-xiguanshuo`習慣說、`chi-ciwuyeti`慈烏夜啼、`chi-chengtian`記承天寺夜遊、`chi-maiyouweng`賣油翁、`chi-shishuo`世說新語選、`chi-kuafu`誇父逐日、`chi-qinxun`勤訓、`chi-gushi19`古詩十九首選、`chi-tangshi`唐詩選、`chi-niao`鳥、`chi-shengyinzhong`聲音鐘），共 **335 題**，全部 `ready:true`。
- ⚠️ **課次分配尚未驗證**：三家 course 的七下目前只有一個大章「**第二冊 經典選文（課次待確認）**」，15 課全掛在下面。原因是家長提供的七下目次與七上真截圖互相矛盾（同一課同時出現在上下學期），故只先建**內容確定**的經典選文，待取得可靠七下目次再拆進各家正確課次。
- 全庫：**146 個可玩單元、0 準備中**。

### 15.【2026-07-22】★第一批非國一內容上線：國二上／國三上 **數學** 9 單元（正好驗證 §10.2 的自動長出）
這是整個題庫**第一次出現國一以外的年級**，剛好把 §10.2「加國二/國三會自動長出年級分頁」實測一遍——遊戲端一行都不用改，重讀 `units.json` 即可。
- **新增 9 個 unit**（皆 `grade:"國二"`/`"國三"`、`term:"上"`、`ready:true`）：
  - 國二上：`mult-formula`乘法公式、`poly-ops`多項式運算、`square-root`平方根與根式、`pythagoras`畢氏定理、`factoring`因式分解、`quadratic-eq`一元二次方程式
  - 國三上：`similar-fig`相似形、`circle-angle`圓、`quad-func`二次函數
  - 全是 keypad 數字題（同 `signed-muldiv` 引擎），答案皆整數；每題經 Node 獨立算法對照驗算 20000 次、瀏覽器實跑 40 題全對。
- **`units.json` 結構微調（遊戲端請留意）**：
  - `courses[]` 每筆**新增 `grade` 欄**（`"國一"/"國二"/"國三"`）。既有 21 筆課程一律補成 `"國一"`；新增三家 `*-g8a`（國二）、`*-g9a`（國三）數學課程各含 `chapters`。
  - 數學章節三家一致（全國課綱鎖死），故 g8a/g9a 三家章名相同——這是正確的，不是漏填。
- **網頁端 parent.html**：頂端多了「國一／國二／國三」年級切換列（**從內容自動長出**，跟 §7 學期鍵同理）；選國二/國三時只顯示該年級有內容的科目（目前僅數學）。與 §10.2 遊戲端邏輯一致。
- 全庫：**155 個可玩單元、0 準備中**（國一 146 ＋ 國二 6 ＋ 國三 3）。

### 15b.【2026-07-22 追加】國二上 **理化** 計算題 3 單元＋新增 `理化` 科目
- **新增科目**：`subjects[]` 多一筆 `{"id":"理化","icon":"⚗️","color":"#db2777"}`。國二起自然科由生物換成理化（國三會再加地球科學）。
- **新增 3 個 unit**（`grade:"國二"`、`term:"上"`、`ready:true`，keypad 計算題、Node 驗算 20000 次＋瀏覽器實跑 40 題全對）：`density`密度、`heat`熱量計算(水，比熱=1，物理正確不虛構)、`wave-speed`波速與聲音(v=fλ＋回聲)。
- **新增三家理化國二課程** `*-sci-g8a`，含 3 章（科學測量與密度／溫度與熱／波動與聲音）。
- 全庫：**158 個可玩單元、0 準備中**。

### 15c.【2026-07-22 完成】理化八上**照真三家目次補完**＋歷史八上上線（家長提供三家目次）
家長提供了理化八上（六章）與歷史八上（四章）三家真目次，已照真結構做完：
- **理化八上 = 三家真六章**（各家章名照對照表）：①測量②物質③聲波④光學⑤熱學⑥微觀。單元 7 個全 `ready`：
  - 計算題（Node 驗算）：`density`密度、`concentration`重量百分濃度、`wave-speed`波速與聲音、`heat`熱量計算
  - 概念題（生成→嚴格審題，答案位置已打散平均）：`sci-matter`物質的世界、`sci-light`光學、`sci-atom`元素與原子
- **歷史八上（中國與東亞史）= 三家真四章**，4 課全 `ready`（生成→審題）：`hist2-shanghan`商周至漢代、`hist2-weitang`魏晉至隋唐、`hist2-songyuan`宋元時期、`hist2-mingqing`明清時期。內容走108課綱「主題式脈絡」（東亞文化圈、經濟重心南移、大航海白銀等），非年號死背。
- 新增三家 `*-sci-g8a`（理化）、`hist-*-g8a`（歷史）課程，chapters 照各家真章名。
- 全庫：**166 個可玩單元、0 準備中**（國一 146＋國二 17＋國三 3）。
### 15d.【2026-07-22 追加】國三上理化力學 4 單元＋國二上英文文法 6 單元＋**修掉一個吃題目的舊 bug**
- **國三上理化（力學）4 單元**（keypad 計算，Node 驗算 20000 次＋瀏覽器各 40 題全對）：`motion`速度與平均速度、`acceleration`加速度、`force-motion`力與運動(F=ma)、`work-power`功與功率。新增三家 `*-sci-g9a` 課程。
  - ⚠️ 章節分法（直線運動／力與運動／功與能）為**暫定標準三章**；內容零風險（公式全國一致），但各家精確章序（尤其「電學」在九上或九下）待真目次校正。
- **國二上英文文法 6 單元**（生成→嚴格審題）：`eng2-past`過去式、`eng2-pastprog`過去進行式、`eng2-future`未來式、`eng2-compare`比較級與最高級、`eng2-infger`不定詞與動名詞、`eng2-conj`連接詞與附加問句。新增三家 `eng-*-g8a` 課程，單一章「文法總複習」——**刻意不宣稱出版社課次結構**（八上文法為課綱鎖死內容），待拿到三家課次目次再加課次章節。
- 🐛 **修掉舊 bug（會影響既有內容，請重讀 units.json 與英文單元）**：組裝器以「題目文字」去重，導致多題共用同一通用題幹（如 `Which sentence is correct?`）時被誤刪。**全庫掃出 31 個單元、默默少了 59 題**（多為國一/七下英文改錯題）。已修：重複題幹自動補上該題考點（如 `Which sentence is correct? (句子改錯／a-an 冠詞)`），使題幹唯一且對學生更清楚；105 個題庫單元全部重組，題目已還原。例：`eng-h-u2` 由 18 題回到 22 題。
- 全庫：**176 個可玩單元、0 準備中**（國一 146＋國二 23＋國三 7）；其中 142 個選擇題單元共 **3412 題**。

### 15e.【2026-07-22 追加】數學加厚 3 單元＋國二國文語文常識 3 單元
- **數學加厚**（keypad，Node 驗算 20000 次＋瀏覽器各 40 題全對）：`arc-sector`弧長與扇形面積（國三·圓，答案填 π 前係數以避開無理數）、`similar-app`相似形的應用（國三·相似形，周長比／**面積比＝邊長比平方**／比例尺單位換算）、`quad-app`一元二次方程式的應用（國二，連續整數／長方形面積）。已掛進既有 g9a／g8a 對應章節。
- **國二國文語文常識 3 單元**（生成→嚴格審題，答案分布 6,6,6,6）：`chi2-xuci`文言虛詞（之其而以於則乃者也）、`chi2-yiwen`文言文閱讀與翻譯（古今義／句讀／判斷句倒裝省略）、`chi2-bingju`語病與修改。新增三家 `chi-*-g8a` 課程，單一章「語文常識與應用」——**跨版本通用、不綁課文**，且已比對國一 53 個國文單元刻意避開重複（修辭／成語／字音字形／六書書信／詞性標點國一已有）。
  - 品管附記：審題後我另做「選項洩題」檢查，抓到 1 題（古今異義題三個錯誤選項都標「（古：…）」而正解標「（古今皆指山）」，不讀題就能猜中），已改為四選項對稱。
- 全庫：**182 個可玩單元、0 準備中**（國一 146＋國二 27＋國三 9）；145 個選擇題單元共 **3484 題**。

### 15f.【2026-07-22 ★里程碑】**國二上七科全數到齊**（家長提供地理/公民/國文/英文目次）
家長補齊了國二上地理、公民、國文、英文的課程結構，本輪一次補完 22 個單元，**國二從 5 科不完整 → 七科全有**。
- **地理（新科目，0→6 課）**：`geo2-asia`亞洲概述與東亞環境、`geo2-eastasia`東亞的文化與經濟、`geo2-china-nature`中國的自然環境、`geo2-china-econ`中國的經濟與社會發展、`geo2-sea`東南亞、`geo2-southasia`南亞。新增三家 `geo-*-g8a` 課程。
- **公民（新科目，0→6 課）**：`civ2-nation`國家與民主政治、`civ2-govtype`政府的組織與功能、`civ2-central`我國的中央政府、`civ2-local`我國的地方政府、`civ2-election`政治參與與選舉、`civ2-party`政黨與利益團體。新增三家 `civ-*-g8a` 課程。
  - 🔒 **公民特別品管**：生成與審題階段都強制「政治中立」（不得出現真實政黨、現任政治人物、政治褒貶）與「避開易過時的精確法規數字」（投票年齡、席次、門檻），改考制度原理。完成後另跑自動掃描：**政治風險 0 命中、時效數字 0 命中**。
- **國文（3→7 單元）**：新增 4 篇課文 `chi2-zhangshizhi`張釋之執法、`chi2-kongchengji`空城計、`chi2-maishuai`麥帥為子祈禱文、`chi2-tiaotiao`迢迢牽牛星；課程重整為「經典文言文／經典白話文／古典詩選／語文常識與應用」四章。
- **英文（6→9 單元）**：新增 `eng2-weather`天氣與季節、`eng2-transport`交通工具、`eng2-directions`問路與指引方向；課程重整為 **Unit 1–6 課次章節**＋文法總複習（原有的過去式／過去進行式／未來式直接對應 Unit 1／2／4，未重做）。
- **理化補完概念題**：新增 `sci-measure`科學測量與實驗室安全、`sci-sound`聲音的性質、`sci-heat`熱的傳播與溫度，補進第1／3／5章。**六章現在每章都有「概念＋計算」兩種練習**。
- ⚠️ **國文有一組年級歸屬待家長裁決**：愛蓮說、陋室銘、聲音鐘、鳥、慈烏夜啼、差不多先生傳、背影 這 7 篇，`units[].grade` 目前仍是 **國一（下）**（先前依舊資料建置時標註為「課次待確認」），但家長最新資料指出屬 **國二上**。本輪採**非破壞做法**：國二上國文課程直接引用這些既有單元（故國二上課程完整可用），但**未改動它們的 `grade` 欄**（故國一下也不受影響）。遊戲端若依 `grade+term` 分頁，這 7 篇目前仍會出現在「國一下」。待家長決定後再統一調整。
- 全庫：**204 個可玩單元、0 準備中**（國一 146＋**國二 49**＋國三 9）；167 個選擇題單元共 **4021 題**。章節引用完整性檢查 0 錯誤。
### 15g.【2026-07-22】**國三上數學／理化照真目次補完**＋修正一處先前的章節錯置
家長提供國三上數學（3章）與理化（4章）真目次，本輪新增 14 個單元。
- 🔧 **修正錯置（會影響既有內容）**：`quad-func`二次函數原本被放在**國三上第3章**，那是先前無目次時的推測，**錯誤**。九上為「相似形／圓／幾何證明與三心」，二次函數屬**九下**。已改為 `term:"下"`、`ch:"第1章 二次函數"`（`units.json` 與該 HTML 的 `unit-meta` 同步修正）。遊戲端若已快取，請重讀。
- **數學（5→11 單元）**，三章：
  - 第1章 相似形：`prop-segments`平行線截比例線段(1-1)、`similar-criteria`相似三角形的判別 AA/SAS/SSS(1-3)、similar-fig、similar-app
  - 第2章 圓的性質：`circle-line`圓與直線的位置關係(弦心距/切線長,2-1)、`circle-quad`弦切角與圓內接四邊形(2-2)、circle-angle、arc-sector
  - 第3章 幾何證明與三角形的三心：`geo-proof`推理與證明(3-1)、`tri-centers`三角形的三心(3-2)
- **理化（4→12 單元）**，四章：
  - 第1章 直線運動：`free-fall`自由落體與位移(含位移vs路徑長)、motion、acceleration
  - 第2章 力與運動：`newton-laws`牛頓運動定律、force-motion、`circular-gravity`圓周運動與萬有引力
  - 第3章 功與能：work-power、`energy`動能位能與力學能守恆、`simple-machine`簡單機械
  - 第4章 基本電學（原本完全空白）：`static-elec`靜電現象、`ohm-law`歐姆定律、`circuit`串聯與並聯電路
- 品管：計算單元全部 Node 獨立算法對照驗算 20000 次（`simple-machine` 初版斜面/槓桿會產生小數答案，已改為「先選省力倍率再回推重量」保證整除）；瀏覽器實測 `circuit` 40/40、`tri-centers` 40/40。概念單元審題特別針對迷思：作用力反作用力≠平衡力、不可稱「離心力把物體甩出」、AAA/SSA 不能判全等、相似面積比為邊長比平方；完成後自動掃描確認 0 個迷思被設為正解。
- 全庫：**218 個可玩單元、0 準備中**（國一 141＋國二 54＋**國三 23**）；172 個選擇題單元共 **4141 題**。引用完整性 0 錯誤、孤兒單元 0。
### 15h.【2026-07-22】新增 **地球科學** 科目（第 9 個有內容的科目）
- **`subjects[]` 新增一筆**：`{"id":"地球科學","icon":"🌏","color":"#0369a1"}`。國三上自然科除理化外另有地科，段考通常合併測驗。
- **6 個單元全 `ready`**（`grade:"國三"`、`term:"上"`）：`es-structure`地球的構造與板塊運動、`es-rocks`岩石與地表的變化、`es-taiwan`台灣的地質與地質災害、`es-atmosphere`大氣與天氣變化、`es-ocean`海水與海洋運動、`es-astronomy`天文（地球運動/月相/日月食/太陽系）。共 **155 題**。
- 新增三家 `es-*-g9a` 課程，六章一對一。
- 品管：審題階段列出地科高頻迷思清單強制查核（地震**規模**vs**震度**不可混用、潮汐主因是月球引力非地球自轉、**日食在朔／月食在望**不可對調、平流層氣溫隨高度**上升**、波浪傳遞能量而非搬運海水前進、黑潮為**暖流**沿台灣東岸北上、大理岩由石灰岩變質、花崗岩深成顆粒粗/玄武岩噴出顆粒細）；並禁止杜撰精確數據與特定災害年份。完成後自動掃描：**迷思 0 命中、具體年份 0 命中**。瀏覽器實測科目色與引擎正常。
- 全庫：**224 個可玩單元、0 準備中**（國一 141＋國二 54＋國三 29）；178 個選擇題單元共 **4296 題**。引用完整性 0 錯誤、孤兒 0。
### 15i.【2026-07-22】國三上**歷史／公民／地理／英文**照真目次補完（國文尚未完成）
家長提供國三上五科三家目次，本輪完成其中四科共 22 個單元。
- **歷史（世界史）5 課**：`hist3-ancient`亞非古文明與歐洲古典文化、`hist3-medieval`中古西歐與伊斯蘭世界、`hist3-renaissance`文藝復興與宗教改革、`hist3-exploration`歐洲海外探險與海權擴張、`hist3-enlighten`科學革命與啟蒙運動。三家 `hist-*-g9a`，各家章名不同已分別套用；**注意南一第3/4課與康軒/翰林順序相反**（南一 3=近代歐洲的變革與擴張、4=文藝復興與宗教改革），單元對應已跟著調換。
- **公民（經濟學）5 課**：`civ3-choice`機會成本、`civ3-market`供需法則、`civ3-money`貨幣金融與物價、`civ3-labor`勞動與就業、`civ3-trade`國際分工與貿易。三家 `civ-*-g9a`。
- **地理（世界地理）6 課**：`geo3-wasia`西亞與北非、`geo3-africa`漠南非洲、`geo3-europe`歐洲、`geo3-russia`俄羅斯與獨立國協、`geo3-namerica`北美洲、`geo3-samerica`中南美洲。三家 `geo-*-g9a`（翰林第5/6課合稱「美洲（上）／（下）」）。
- **英文（核心文法）6 單元**：`eng3-perfect1`現在完成式(經驗/持續)、`eng3-perfect2`現在完成式(since/for)、`eng3-passive`被動語態、`eng3-noun-clause`名詞子句、`eng3-tag-too`附加問句與too...to、`eng3-conj`連接詞與動名詞。三家 `eng-*-g9a`，Unit 1–6 課次章節（三家文法進度綁定，僅 Unit5/6 名稱略異）。
- 品管掃描全數 0 命中：公民**政治風險 0**、**經濟迷思 0**（機會成本非加總、台幣升貶值未寫反）；史地**具體年份 0**（重脈絡不背年代）；英文**常見錯誤 0**（have gone to 誤用、完成式配過去時間、間接問句倒裝、too...not to）。所有單元答案位置分布平均、無重複題幹與選項。
- 🔧 **課文搬移（會影響既有內容）**：`chi-xiguanshuo`習慣說、`chi-chengtian`記承天寺夜遊、`chi-maiyouweng`賣油翁 原標 **國一下**（在「課次待確認」章），依家長最新目次改為 **國三上**（`units.json` 與各 HTML 的 `unit-meta` 同步）。國一下國文「課次待確認」章現剩 7 篇（木蘭詩、古詩十九首選、唐詩選、座右銘、世說新語選、誇父逐日、勤訓），待七下真目次再拆。
- 全庫：**246 個可玩單元、0 準備中**（國一 138＋國二 54＋國三 54）；200 個選擇題單元共 **4825 題**。引用完整性 0 錯誤、孤兒 0。
- ⏸️ **國三上國文尚未完成**：三家代表選文 11 篇（詩經蒹葭／詞選／楚人養狙／與宋元思書／大明湖／錯誤／青鳥就在身邊／傘季／飛魚／生機／森林最優美的一天）的題庫生成因控管用量而暫停。`chi-*-g9a` 課程已先建立但**僅含已完成的 3–4 篇**，待補齊。
- ⚠️ 仍缺：國三上**國文**（如上）、各年級**下學期**（策略：上學期優先，近開學再補）。
